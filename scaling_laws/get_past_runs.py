import requests

def get_previous_runs(config):
    response = requests.get("http://hyperturing.stanford.edu:8000/previous_runs", config)
    return response.json()

if __name__ == "__main__":
    DEFAULT_API_KEY = """ssh-rsa  ayushalag@Ayushs-MacBook-Pro.local"""

    api_key_config = {
        "api_key": DEFAULT_API_KEY
    }

    previous_runs = get_previous_runs(api_key_config)
    print(previous_runs)